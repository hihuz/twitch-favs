...
/*
  ...
  - should be as passive as possible, handlers need to be there but should delegate the logic to user-input module
  - passes all user actions by way of events to user-input module
*/

//TODO -----------> !!!!!!!! the actual logic for alert close click should be in user-input to be consistent.. !!!!!!!!!!!!!
...
const View = (function() {
  ...
  addContent.addEventListener("transitionend", function(e) {
    addInput.focus();
    addInput.select();
  });

  add.addEventListener("transitionend", function(e) {
  ...
  addBoxValidate.addEventListener("click", function(e) {
  ...
  function buildChannelDOM(chanData, appendFunc) {
    ...
    $(".channel-link", template).addEventListener("click", function(e) {
      e.preventDefault();
      EV.emit("chan-link-click", chanData.channel);
    });
    $(".remove", template).addEventListener("click", function(e) {
  ...
  function filterDOMList(list) {
    $$("main .stream").forEach(function(stream) {
      const chanName = stream.getAttribute("data-channel");
      if (list.indexOf(chanName)==-1) { stream.classList.add("hidden"); }
      else { stream.classList.remove("hidden"); }
    });
  }
