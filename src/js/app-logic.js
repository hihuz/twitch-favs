/*
  ...
  - store object should only be accessed with privileged methods but isn't
    actually private for convenience, this would be updated
...
// this is called everytime a channel in the list is offline
// so we can at least get channel data to display
...
// !!This is better than the other way around coz there
// is no way to know if a channel is online from the channel api!!
...
function fullDataGather(list, cb) {
  ...
  .reduce((chain, streamPromise) => chain
    .then(() => streamPromise)
    .then(cb), Promise.resolve()); // init promise chain
}
...
function getFilteredList(filter, list) {
  const filteredList = list.filter((channel) => {
